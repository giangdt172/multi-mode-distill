...
def topk_kl_loss(student_logits, teacher_logits, token_mask, temperature=1.0):
    if not math.isfinite(temperature) or temperature <= 0:
        raise ValueError("temperature must be finite and positive")
    if student_logits.shape != teacher_logits.shape or student_logits.ndim != 3:
        raise ValueError("Expected aligned top-k logits [B, L, K]")
    if token_mask.shape != student_logits.shape[:2] or student_logits.shape[-1] < 2:
        raise ValueError("Expected token mask [B, L] and at least two candidate tokens")
    student_logprobs = F.log_softmax(student_logits.float() / temperature, dim=-1)
    teacher_logprobs = F.log_softmax(teacher_logits.detach().float() / temperature, dim=-1)
    per_token = (teacher_logprobs.exp() * (teacher_logprobs - student_logprobs)).sum(-1)
    return per_token.masked_fill(~token_mask, 0).sum() / token_mask.sum().clamp_min(1) * temperature ** 2
